...
# Replace the mangled grids completely again
...
fun ScientificButtonGrid(
    ...
    Column(
        ...
        Row(modifier = Modifier.fillMaxWidth().let { if (isLandscape) it.weight(1f) else it }, horizontalArrangement = Arrangement.spacedBy(buttonSpacing)) {
            ...
            CalculatorButton(symbol = "EXP", color = color, textColor = textColor, modifier = baseButtonModifier.weight(1f), onClick = { onEvent(CalculatorEvent.InputString("*10^(")) })
